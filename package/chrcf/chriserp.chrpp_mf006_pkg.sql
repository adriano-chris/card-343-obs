...
Procedure Atualiza_Rastreabilidade(P_Sq_Lote In Pd_Lote_Rastrea.Sq_Lote%Type)
   ...
   Begin
     ...
     -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
     -- Finalidade: Apontamento Automático SAP Via Web Service
       ...
       If ( T.Check_Integracao_SAP = 0 ) Then
          ...
          Begin
            ...
            /*   Begin
               ...
               V_Qt_Baixa := V_Qtde_Aux; --( V_Qtde_Aux - V_Qt_Contador );

               /*
               dbms_output.put_line(
               'V_Table_Plr.Maquina_Cod'||V_Table_Plr.Maquina_Cod||chr(13)||--pd_lote_rastrea.Maquina_Cod%Type
               'r1.Nr_Op: '             ||r1.Nr_Op     ||chr(13)|| --Pd_Lote_Op.Nr_OP%Type
               'P_Apontamento: '        ||V_Apontamento||chr(13)|| --Pd_Op_Apontamento.Sq_Op_Aponta%Type --PK
               'P_Qtde: '               ||V_Qt_Baixa   ||chr(13)|| --pd_op_apontamento.qt_contador%Type
               'P_Produto: '            ||V_Cod_Produto||chr(13)|| --pd_lote_rastrea.Part_No%Type
               'V_Cd_Fam: '             ||V_Cd_Fam     ||chr(13)|| --pd_lote_rastrea.cd_fam%Type
               'T.Sq_Lote: '            ||T.Sq_Lote );             --pd_op_apontamento.Sq_Lote%Type
               */
               --
               --
               Begin
               ...
               If ( T.Check_Maquina > 0 ) Then
                   ...
                   -- DEVE SER UTILIZADAPA PARA ADICIONAR NOVAS MAQUINAS NO PROCESSO DE APONTAMENTO --
                   -----------------------------------------------------------------------------------
                   For Reg In
       ...
       Exception
         When No_Data_Found Then
           ...
           dbms_output.put_line(
              ...
              'P_Nr_Op: '         || LPAD(P_Nr_Op, 12, '0')||chr(13)||
              'P_sq_controle_op: '|| P_sq_controle_op      ||chr(13)||
              'Procurar por (Data_Fim Sintético)' );
           */
       End;
       -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
  ...
  FUNCTION Fun_Calc_Generico (
    ...
    Begin
      ...
      --
      --
      Elsif ( P_Calc = 5 ) THEN  -- Disponível p/ implementação....
       Null;
      --
      --
